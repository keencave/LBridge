...
// sensor information buffer sizes
#define SNLEN           20        // length of decoded sensor number
#define SSBYTE          20        // length of sensorStatusByte message

#define TIMESTRLEN      20        // lenght of serial output time string
#define TXBUFLEN        30        // length of LimiTTer style output string via ble
...
#define RAW2DEX         (8.5f)    // scale factor to transfer raw libre readings
                                  // to xDrip+ scale
...
#else
#define MY_FLASH_PAGE  235        // to enable OTA programming, 230/235/237 usable accorind @FPV-UAV
#endif
...
char timestring[TIMESTRLEN];        // output buffer len for print_state() command
...
static boolean BatteryOK = false;   // is battery voltage OK for operation

char TxBuffer[TXBUFLEN];            // BLE send buffer
...
static boolean sreached = 0;        // flag if stack limit is reached / check_stack()
...
static boolean bleStackStarted = 0; // is BLE stack started? V0.9.19

//static boolean tlb = 0;           // for test simulating an empty battery
...
#endif
#ifdef USE_XBRIDGE2
  Serial.printf("-xbridge2");
#endif
#ifdef USE_SHADOWFRAM
  Serial.printf("-shadowFRAM");
#endif
#ifdef SHORTSTARTCYCLE
  Serial.printf("-10cycles1min");
#endif
#ifdef SERIALOFF
  Serial.printf("-serialOff");
#endif
#ifdef USE_WDT
  Serial.printf("-WDT");
#endif
...
void loop()
{
  ...
  #ifdef USE_FLASH
    if ( runPeriod != p->runPeriod )  runPeriod = p->runPeriod;
  #else
    if ( runPeriod != 5 )             runPeriod = 5;
  #endif
    print_state("");
  ...
  if ( BatteryOK ) {
    readAllData();
    if (NFCReady == 2) {
        currentBG = true;         // MM add for G4 back-compatibility
#ifdef USE_XBRIDGE2
    ...
    // switch OFF BLE stack to avoid that device is still dectable as user assumes it is running
    // check for endless loop / V0.9.19
    if ( bleStackStarted ) {
      print_state(" *** BLE stack is ON, switch OFF");
#ifdef RFD    
...
//  print_state(" *** wait 5 sec for serial key");
...
void initialFillupBackfillTimestamps(void)
{
  ...
  store = sensorData.sensorStatusByte;
   // needed for using the put_reading2queue() function without valid sensor data
  sensorData.sensorStatusByte = 0x03;   // 0x03 = sensor is running normal
...
void configWDT()
{
  ...
  NRF_WDT->CRV = 32768 * 60 * 11;         // 11 minutes
...
bool idnResponseHasNoError()
{
  ...
  if (resultBuffer[0] == 0x00)  return true;
  else                          return false;
}
...
//  check_stack("#rspHsNoErr#");
  ...
  if (resultBuffer[0] == 0x80) {
    if ((resultBuffer[2] & 0x01) == 0)  return true;
  }
  return false;
}
...
int controlProtocolService()
{
  ...
  while ( (Serial.available() || bleCharAvailable()) && command_buff.nCurReadPos < COMMAND_MAXLEN) {
    ...
    if ( bleCharAvailable() ) {
      ...
      b = Serial.read();
//      print_state("");
//      Serial.printf("serial char received: <%c>", b);
    }
...
int doCommand(void)
{
  ...
  if (command_buff.commandBuffer[0] == 0x02 && command_buff.commandBuffer[1] == 0xF0 && !got_ack) {
    #ifdef DEBUG
    print_state("");
    Serial.printf("got ACK packet for 0x%x - ", dex_tx_id);
    #endif
    got_ack = 1;
    init_command_buff(&command_buff);
    return (0);
...
//    #ifdef DEBUG
      print_state("");
      Serial.printf("R-command received - do BG scan");
//    #endif  
...
void sendToXdripViaBLE(void)
{
  ...
  while ((Pkts.read != Pkts.write) && BTconnected && ((mymillis() - pkt_time) < ((DXQUEUESIZE + 1) * 5000) )) {
    ...
    if ( Pkts.buffer[Pkts.read].raw ) {
      ...
      if ( more_than_one++ )  waitDoingServices(100, 1);
...
//      Serial.printf("%d - ", Pkts.read);
      ...
      if ( !resend_pkt ) {
        ...
        if ( BTconnected )    waitDoingServices(5000, 1);
      }
      else {
...
//  msg.dex_battery = 214;  // simulate good dexcom transmitter battery
  ...
  *(unsigned char *)&msgbuf[20] = msg.function;
  ...
  if (currentBG)
  {
  *(unsigned char *)&msgbuf1[0] = 17;
  *(unsigned char *)&msgbuf1[1] = msg.cmd_code;
  *(unsigned long *)&msgbuf1[2] = scale_bg(sensorData.trend[0]);
  *(unsigned long *)&msgbuf1[6] = scale_bg(sensorData.trend[0]);
  *(unsigned char *)&msgbuf1[10] = msg.dex_battery;
  *(unsigned char *)&msgbuf1[11] = msg.my_battery;
  *(unsigned long *)&msgbuf1[12] = msg.dex_src_id;
  *(unsigned char *)&msgbuf1[16] = msg.function; 
  ...
  if (!BTconnected)
  {
    if ( BatteryOK ) {
      ...
      if ( bleStackStarted ) {
        ...
        print_state(" *** do not need to stop BLE stack");
      }
      BTconnected = false;
      ...
      print_state("do not do BLE restart as battery is empty");
    }
  }
}
...
void RFduinoBLE_onReceive(char *data, int len)
{
  ...
  // MM
  waitDoingServices(200, 1);      // process any commands sent via BLE
}
...
//  if ( tlb ) {
//    print_state(" *** simulate battery empty");
//    BatteryOK = false;
//  }
}
