...
#define N_USE_SMOOTHING       // use NFC data smoothing (LimiTTer method)?
#define DB_PKT                // debug infos for queue handling
...
#define LNAME     "LimiTTer"    // BLE name of the module
#define LVERSION  "0627_0011"  // Versionnumber

// less common settings
#define N_INDICATE_NFC_ERROR    // store misased NFC reading as BG value 1 to see it in xDrip+
#define N_SHOW_VOLTAGE          // display voltage statistics after every loop, toogle init value
#define N_UPDATE_HM1X           // update HM-11
...
float Read_Memory(boolean *readDone)
{
  ...
  if (RXBuffer[0] == 128) {
    ...
    //    print_state(F(" - Glucose pointer: ")); Serial.print(glucosePointer);

    int ii = 0;
    // read all blocks, one block is 12 ASCII
...
*/
    ...
    // print 15 min trend data, only linear, but should be descending beginning at glucosepointer!!!
    //     print_state(F("trend[] : "));
    ...
    // VW always the raw value
/*    
    print_state(F(" - no LimiTTer data smoothing: "));
...
#ifdef SHOW_MEM
    print_state(F(" - avail. mem: ")); Serial.print(freeMemory());
#endif
    /*
...
#ifdef UPDATE_HM1X
  if ( toupper(command_buff.commandBuffer[0]) == 'U' ) {
...
#endif
#ifdef DB_PKT
  if ( toupper(command_buff.commandBuffer[0]) == 'X' ) {
...
void print_statistics2()
{
  ...
  switch ( loop_state ) {
    case NORMAL:
      ...
      break;
    case ST_RESEND:
      print_state(F(" XXX loop ")); Serial.print(++loop_count); Serial.print(F(" XXX "));
      Serial.print(F("1ST_RESEND"));
      firstret_loop_count++;
      break;
    case ND_RESEND:
      print_state(F(" YYY loop ")); Serial.print(++loop_count); Serial.print(F(" YYY "));
      Serial.print(F("2ND_RESEND"));
      secondret_loop_count++;
      break;
    default:
      print_state(F(" ZZZ unknown state "));
  ...
  if ( loop_count >= 2 ) {
    Serial.print(F("\r\n *** avg. sleep time: "));  Serial.print(avg_sleepTime); Serial.print(F(" ")); Serial.print(avg_sleepTime / (loop_count-1));
    Serial.print(F(" s ProMini+BLE On: "));  Serial.print(bleTime); Serial.print(F(" ")); Serial.print((bleTime / (loop_count-1)) / 1000);
...
void loop(void)
{
  ...
  */
  ...
  while (1)
  {
    ...
    if ( (loop_count % 12) == 0 ) {
      print_state(F(" - store voltage statistics"));
      if ( voltage_wi >= MAX_VOLTAGE )
        voltage_wi = 0;
      batt_voltage[voltage_wi].voltage = (batteryMv/10)-200;
      batt_voltage[voltage_wi].a_on = (((float)arduino_ontime / (prg_run_time*1000))*100)+0.5;
      batt_voltage[voltage_wi].b_on = (((float)ble_ontime     / (prg_run_time*1000))*100)+0.5;
      batt_voltage[voltage_wi].n_on = (((float)nfc_ontime     / (prg_run_time*1000))*100)+0.5;
      voltage_wi++;
    }

#ifdef AUTOCAL_WDT
    // recalibrate PWR DWN timer every 30 m, loop duration is 5 m
    if ( (loop_count % 6) == 0 ) {  // every 5 loops
      print_state(F(" - calibrate PWR DOWN sleep timer (2048 ms) ..."));
      calibrate(2048);
    }
#endif
...
//      x_command = 0;
    ...
    if ( v_command ) {
      print_state(F(" - voltage in 1h spacing - "));
      int i;
      for ( i = 0 ; i < MAX_VOLTAGE; i++ ) {
        if ( ( i % 4 ) == 0 )
          Serial.print(F("\r\n")); 
        if ( batt_voltage[i].voltage != 0 ) {
          Serial.print((batt_voltage[i].voltage+200)*10); Serial.print(F(" mV "));
/*
