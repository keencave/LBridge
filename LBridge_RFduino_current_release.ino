...
/*#
 * changes since LBridge_RFduino_70729_1647.ino
...
#define N_VW                      // test setup
#define N_SIM_MINUTES_UPCOUNT
...
#ifdef VW
#define DXQUEUESIZE (1*12) // 1 h of queue
#else
...
void setup()
{
  ...
  print_state("setup()");
  print_state(" ------------------------------------------------------------------");
  print_state(" --- Test LBridge starting ---");
...
void show_trend_history(void)
{
  ...
  for (int i = 0; i < 16; i++) {
    // 1 min distance
    Serial.printf(" %f (-%d s)", (float)scale_bg(sensorData.trend[i]), i*60);
    if ( i > 0 && !(i%4) )
      Serial.println("");
  }
  ...
  for (int i = 0; i < 32; i++) {
    Serial.printf(" %f (-%d s)", (float)scale_bg(sensorData.history[i]), (sensorData.minutesHistoryOffset)*60+(15*60*i));
    if ( i > 0 && !(i%4) )
      Serial.println("");
  }
}

void fillup_missing_readings(void)
{
  int i, j;
  int nulls = 0;
  int difftime;

  print_state("fillup_missing_readings() - ");
  for ( int i = 0 ; i < DXQUEUESIZE ; i++ ) {
    if ( (Pkts.buffer[i].raw == 0) && (Pkts.buffer[i].ms != 0) ) {
      Serial.print("\r\nfound missed readings at");
      Serial.printf(" index %d, raw = %d, time = %d s", i, Pkts.buffer[i].raw, (Pkts.buffer[i].ms+500)/1000);
      nulls++;
      // if the missing value is not longer than 15 min old search in trend array
      if ( (millis()-Pkts.buffer[i].ms) <= (15*60*1000) ) {
        print_state("searching in trend array - ");

        Serial.printf("search for value at %d ms (%d s)", Pkts.buffer[i].ms, (Pkts.buffer[i].ms+500)/1000);
        Serial.printf("\r\ncurrent time since program start:", minutesSinceProgramStart*60);
        for ( j = 0 ; j < 16 ; j++ ) {
          print_state("trying "); Serial.printf("-%d s, %d, %d", j*60, (minutesSinceProgramStart-j)*60, (Pkts.buffer[i].ms+500)/1000);  
          // trend array begins at current time - counting in minutes
          difftime = ((minutesSinceProgramStart-j)*60)-((Pkts.buffer[i].ms+500)/1000);
          Serial.printf(" = %d", difftime);
          difftime = abs(difftime);
          Serial.printf(",  abs() = %d", difftime);
          if ( difftime < 31 ) {
            print_state("found at "); Serial.printf("%d s, original timestamp %d s", j*60, (Pkts.buffer[i].ms+500)/1000);
            Serial.printf("\r\nreplacing %d with %d", Pkts.buffer[i].raw, scale_bg(sensorData.trend[j]));
...
//            break;
        ...
        print_state("searching in history array - ");

        Serial.printf("search for value at %d ms (%d s)", Pkts.buffer[i].ms, (Pkts.buffer[i].ms+500)/1000);
        Serial.printf("\r\ncurrent time since program start:", minutesSinceProgramStart*60);
        for ( j = 0 ; j < 32 ; j++ ) {
          print_state("trying "); Serial.printf("-%d s, %d, %d", j*60, (minutesSinceProgramStart-(sensorData.minutesHistoryOffset+15*j))*60, (Pkts.buffer[i].ms+500)/1000);  
          difftime = ((minutesSinceProgramStart-sensorData.minutesHistoryOffset-(j*15))*60)-((Pkts.buffer[i].ms+500)/1000); 
          if ( abs(difftime) < 451 ) {
            print_state("found at "); Serial.printf("%d s, original timestamp %d s", j*60, (Pkts.buffer[i].ms+500)/1000);
            Serial.printf("\r\nreplacing %d with %d", Pkts.buffer[i].raw, scale_bg(sensorData.history[j]));
  ...
  Serial.print(", missed backfilling values detected: "); Serial.print(nulls);
}

void loop()
{
  Serial.printf("\r\n=== loop #%d === BT status %d", ++loop_cnt, BTconnected);
  Serial.printf(" === RSSI: %f === %d mV ==============================================", SoCData.rssi, SoCData.voltage);
...
void printIDNData(IDNDataType idnData)
{
  ...
  print_state("NFC Device ID [char]: ");
  Serial.print(nfc);
...
void SoC_Data()
{
  ...
  print_state("raw analog Vin : "); Serial.print(sensorValue);
...
void apply_spike_filter(void)
{
  ...
  if ( ((lastGlucose-currentGlucose) > SPIKE) || ((currentGlucose-lastGlucose) > SPIKE) ) {
...
//  print_state("readAllData()");
  ...
  if ( sensorData.sensorDataOK ) {
    ...
    put_reading2queue(sensorData.trend[0]);
  }
  else {
...
int put_reading2queue(int glucose)
{
  // read the current BG from Libre sensor
  get_packet(&Pkts.buffer[Pkts.write], glucose);

#ifndef USE_DEAD_SENSOR
  // test for dead sensor
  ...
  // sensor is dead either minutes elapsed reached or 5 succeeding equal values read
  if ( sensorData.minutesSinceStart > MAX_SENSOR_MINUTES ) {
    print_state(" *** sensor livetime of "); Serial.print(sensorData.minutesSinceStart);
    Serial.print(" > "); Serial.print(MAX_SENSOR_MINUTES); Serial.print(", termintated");
    Pkts.buffer[Pkts.write].raw = 0;        
  }
  if ( noDiffCount > 5 ) {
    print_state(" *** sensor dead, 5 succeeding equal values of "); Serial.print(sensorData.trend[0]);
    Pkts.buffer[Pkts.write].raw = 0; 
  }
#endif

  print_state("got packet with "); Serial.print(glucose); 
  Serial.printf(", stored at position %d, incrementing write to ", Pkts.write);
...
void show_raw_trend_history_data(void)
{
  print_state("raw body data 24 ... 320");
  int i;
  for ( i = 0 ; i < (320-24) ; i++) {
    if ( (i%8) == 0 )
      Serial.println(F(""));
...
/*
  Serial.print("index/pointer/minutes:");
...
*/
  ...
  for (int i = 0; i < 16; i++)                              
  {
    ...
    Serial.printf(" #[%d][%x](%d):\t", i, sensorData.trend[i], sensorData.trend[i]); 
...
void displaySensorData()
{
  ...
  {
    print_state("BG reading: "); Serial.print(sensorData.trend[0]);
    Serial.print(", sensor S/N "); Serial.print(sensorData.sensorSN);
    Serial.print(", Sensor status: "); Serial.print(sensorData.sensorStatusByte, HEX); 
    Serial.print(" - "); Serial.print(sensorData.sensorStatusString);
    ...
    print_state("current BG reading: "); Serial.print(sensorData.trend[0]);
...
/*
void print_statef(const char *format, ...)
{
  va_list vl;
  size_t n = 0;
...
{
  print_state("get_packet() with BG "); Serial.printf("%d (%d) - ", actGlucose, scale_bg(actGlucose));
//  pPkt->raw = actGlucose * 100;
  pPkt->raw = scale_bg(actGlucose);
  pPkt->ms = millis();
  pkt_time = pPkt->ms;
  Serial.printf("packet read at %d (%d ms)", (pPkt->ms+500)/1000, pPkt->ms);
  /*
...
int print_packet(Dexcom_packet* pPkt)
{
  ...
  print_state("print_packet() with a delay of "); Serial.print(millis() - pPkt->ms);
